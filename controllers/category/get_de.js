...
const { Category_de, Catalog } = require("../../models");

const get_de = async (req, res, next) => {
  const today = new Date().getDate();
  try {
    const categoryFilters = await Category_de.find();
    ...
    //   });
    // }
    // await Category_de.findOneAndDelete({ _id: categoryFilters[0]?._id.toString()
    // });
    // const category = await Category_de.create(filterParams);
