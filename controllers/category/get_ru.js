...
const { Category_ru, Catalog } = require("../../models");

const get_ru = async (req, res, next) => {
  const today = new Date().getDate();
  try {
    const categoryFilters = await Category_ru.find();
    ...
    //     }
    //     if (level_3) {
    //       if(!filterParams.level_3[`${level_2}`]){filterParams.level_3[level_2] =[]}};
    //     level_3.map(it => {if(!filterParams.level_3[level_2].includes(it)){
    ...
    //   });
    // }
    // await Category_ru.findOneAndDelete({ _id: categoryFilters[0]?._id.toString()
    // });
    // const category = await Category_ru.create(filterParams);
