...
const { Catalog, Test_ru, Test_ua } = require("../../models");

const getShop_ru = async (req, res, next) => {
  const usd = 39;
  const euro = 42;
  try {
    const shop_ru = await Test_ru.find();
    const shop_ua = await Test_ua.find();

    let array_ru = [...shop_ru];
    ...
    array_ua.map((key) => {
      array_ru.map((it) => {
        if (key.uuid === it.uuid) {
          ...
          Test_ru.deleteOne({ uuid: it.uuid });
          Test_ru.create(it);
        }
      });
    });
    ...
    // Test_ru.create(item)
    //   })
    const shop = await Test_ru.find().limit(12);
